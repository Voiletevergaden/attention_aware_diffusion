...
class LossFunctions:
    ...
    def log_normal(self, x, mu, var):  # 对数概率密度函数（log-PDF） 的计算，先验概率的 log-likelihood

        if self.eps > 0.0:
            var = var + self.eps
        return -0.5 * torch.mean(
            torch.log(torch.FloatTensor([2.0 * np.pi]).cuda()).sum(0) + torch.log(var) + torch.pow(x - mu, 2) / var,
            dim=-1)
